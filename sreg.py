...
def check(plugin, passport, passport_type):
    ...
    if plugin["request"]["method"] == "GET":
        try:
            ...
            s = requests.Session()
            s.headers = headers
            r = s.get(url, headers={}, timeout=8)

            html = r.text.encode(r.encoding or "utf8").decode("utf-8")
        except Exception as e:
            print(inRed("\n[-] %s Error: %s\n" % (app_name, str(e))))
            return
        if judge_yes_keyword in html and judge_no_keyword not in html:
            print(u"[{0}] {1}".format(category, ("%s (%s)" % (app_name, website))))
            icon = plugin["information"]["icon"]
            desc = plugin["information"]["desc"]
            output_add(
                category, app_name, website, passport, passport_type, icon, desc,
            )
        else:
            pass
        ...
        try:
            s = requests.Session()
            s.headers = headers
            r = s.post(url, data=post_data, headers={}, timeout=8)
            html = r.text.encode(r.encoding or "utf8").decode("utf-8")
        except Exception as e:
            print(inRed("\n[-] %s Error: %s\n" % (app_name, str(e))))
            return
    ...
    for plugin in plugins:
        ...
        if parser_argument.cellphone:
            p = multiprocessing.Process(
                target=check, args=(content, parser_argument.cellphone, "cellphone",),
            )
        elif parser_argument.user:
            p = multiprocessing.Process(
                target=check, args=(content, parser_argument.user, "user"),
            )
        elif parser_argument.email:
            p = multiprocessing.Process(
                target=check, args=(content, parser_argument.email, "email"),
            )
        p.start()
        jobs.append(p)
